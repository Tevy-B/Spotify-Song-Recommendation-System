...
def display_recommendations(response):
    ...
    for idx, track in enumerate(response['tracks']):
        lbl_track_name_recommendation = create_label(track['name'])
        lbl_track_name_recommendation.grid(row=idx + 1, column=0)
        lbl_artist_name = create_label(track['artists'][0]['name'])
        lbl_artist_name.grid(row=idx + 1, column=1)
        btn_play_it_recommended = create_button(
            'Play It', track['external_urls'])
        btn_play_it_recommended.grid(row=idx+1, column=2, padx=10)

        def get_recommendations():
            search = ent_search.get()

            # Get your Spotify ClientId and ClientSecret :
            ...
            btn_get_recommendations = tk.Button(
                master=frm_search_field, text='Get recommendations', command=get_recommendations)
